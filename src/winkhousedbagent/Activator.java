package winkhousedbagent;

import java.io.IOException;
import java.util.Date;

import org.eclipse.jface.resource.ImageDescriptor;
import org.eclipse.ui.plugin.AbstractUIPlugin;
import org.osgi.framework.BundleContext;

import winkhousedbagent.helper.BackUpHelper;
import winkhousedbagent.util.WinkhouseDBAgentUtils;

/**
 * The activator class controls the plug-in life cycle
 */
public class Activator extends AbstractUIPlugin {

	// The plug-in ID
	public static final String PLUGIN_ID = "winkhouseDBAgent";

	// The shared instance
	private static Activator plugin;
	
	/**
	 * The constructor
	 */
	public Activator() {
	}

	/*
	 * (non-Javadoc)
	 * @see org.eclipse.ui.plugin.AbstractUIPlugin#start(org.osgi.framework.BundleContext)
	 */
	public void start(BundleContext context) throws Exception {
		super.start(context);
		plugin = this;
//		BackUpHelper bUpHelper = new BackUpHelper();
//		bUpHelper.backuptaskStatus();
	}

	/*
	 * (non-Javadoc)
	 * @see org.eclipse.ui.plugin.AbstractUIPlugin#stop(org.osgi.framework.BundleContext)
	 */
	public void stop(BundleContext context) throws Exception {
//		if (WinkhouseDBAgentUtils.getInstance().getBackupThread() != null){
//			WinkhouseDBAgentUtils.getInstance().getBackupThread().cancel(true);
//		}
//		WinkhouseDBAgentUtils.getInstance().getPreferenceStore().setValue(WinkhouseDBAgentUtils.LAST_START_BACKUPDATI, new Date().toString());
//	    try {
//			WinkhouseDBAgentUtils.getInstance().getPreferenceStore().save();
//		} catch (IOException ioe) {
//			ioe.printStackTrace();
//		}						
		plugin = null;
		super.stop(context);
	}

	/**
	 * Returns the shared instance
	 *
	 * @return the shared instance
	 */
	public static Activator getDefault() {
		return plugin;
	}

	/**
	 * Returns an image descriptor for the image file at the given
	 * plug-in relative path
	 *
	 * @param path the path
	 * @return the image descriptor
	 */
	public static ImageDescriptor getImageDescriptor(String path) {
		return imageDescriptorFromPlugin(PLUGIN_ID, path);
	}
}
